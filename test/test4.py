"""
Approximation of MMD_u^2 with the limit distribution (Gretton 2012).
This limit distribution needs the eigenvalues of a functional operator. Doing some trickery, I think we can
    approximate them with the eigenvalues of the Gram matrix of a sample.
"""
...
for kernelname, (kernel, kernelargs, epsstar, deltastar) in KERNELS.items():
    for n in tqdm(range(2, nv // 2 + 1)):
        ...
        for i in range(rep):
            sub1, sub2 = rankings.get_subsamples_pair(subsample_size=n, seed=SEED + i, use_rv=True, use_key=False,
                                                      replace=False)
            ...
            mmdu2 = np.sum(ugram11) / (n*(n-1)) + np.sum(ugram22) / (n2 * (n2 - 1)) - 2 * np.mean(gram12)
...
reload(ru)
rankings = ru.SampleAM(rankings)

n = 25  # for MMDu2
rep = 500  # number of samples for MMDu2
neig = 200  # number of approximate eigenvalues of functional operator
nsample = 200  # to center the gram matrix
...
sub = rankings.get_subsample(subsample_size=neig, seed=15151515, replace=True)  # approximate eigenvalues
sub5 = rankings.get_subsample(subsample_size=nsample, seed=141414, replace=True)  # center K
sub6 = rankings.get_subsample(subsample_size=nsample, seed=131313, replace=True)  # expected k
sub7 = rankings.get_subsample(subsample_size=nsample, seed=131313, replace=True)  # expected k

# first term in (9)
K = ku.square_gram_matrix(sub, use_rv=True, kernel=kernel, **kernelargs)

# second and third terms in (9)
b = np.array([[kernel(xi, y, use_rv=True, **kernelargs) for y in sub5] for xi in sub]).mean(axis=1)
B1 = np.ones_like(K) * b
B = B1 + B1.T

# fourth term in (9)
c = np.mean(ku.gram_matrix(sub6, sub7, use_rv=True, kernel=kernel, **kernelargs))
C = np.ones_like(K) * c

# matrix we compute eigenvalues on
Ksim = K - B + C

Ksim = Ksim - np.diag(Ksim.diagonal())

# eigenvalues of the centered Gram matrix
eigs = np.linalg.eigh(Ksim)[0]  # for Hermitian matrices

# --- Approximate MMDu2
rng = np.random.default_rng(121212)
tmp = []
for eig in eigs:
    al = rng.normal(loc=0, scale=1, size=nnormal)
    bl = rng.normal(loc=0, scale=1, size=nnormal)
    tmp.append(eig * (2 * (al-bl)**2 - 4))
mmdu2_approx = 1/(2*n) * np.mean(tmp, axis=0)
...
ax = axes[2]
sns.histplot(eigs, ax=ax, stat="probability", bins=100)
ax.set_xlabel("Eigenvalues of centered Gram matrix")
