...
s1 = distr.sample(2000)
s2 = distr.sample(2000)
...
def gram_matrix_am_mallows(s1, s2, nu):
    ams1 = list(ru.AdjacencyMatrix.from_bytes(r, shape=(na, na)) for r in s1)
    ams2 = list(ru.AdjacencyMatrix.from_bytes(r, shape=(na, na)) for r in s2)

    tmp = np.array(list(product(ams1, ams2)), dtype=int)  #TODO can be faster, avoid using product
    return np.exp(-nu / 2 * np.logical_xor(tmp[:, 0], tmp[:, 1]).sum(axis=(1, 2)).reshape(len(ams1), len(ams2)))
...
# g2 = gram_matrix_am_mallows(s1, s2, nu=nu)
...
g2 = np.exp(-nu / 2 * np.logical_xor(tmp[:, 0], tmp[:, 1]).sum(axis=(1, 2)).reshape(len(ams1), len(ams2)))
...
def test_subsample_mmd_distribution(sample: ru.SampleAM, subsample_size: int,
    ...
    for ir in range(rep):
        ...
        kxx = gram_matrix_am_mallows(ams1, ams1, nu=nu)
        kxy = gram_matrix_am_mallows(ams1, ams2, nu=nu)
        kyy = gram_matrix_am_mallows(ams2, ams2, nu=nu)
...
tm0 = time.time()
mmd.subsample_mmd_distribution(sample=s1, seed=1444, subsample_size=50, rep=1000, use_rv=True, kernel=ku.mallows_kernel)
tm1 = time.time()
test_subsample_mmd_distribution(sample=s1, seed=1444, subsample_size=50, rep=1000, use_rv=True)
tm2 = time.time()
